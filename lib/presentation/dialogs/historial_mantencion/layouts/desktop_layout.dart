...
class HistorialMantencionDesktopLayout extends StatefulWidget {
  final Bus bus;

  HistorialMantencionDesktopLayout({required this.bus});

  @override
  _HistorialMantencionDesktopLayoutState createState() =>
      _HistorialMantencionDesktopLayoutState();
}

class _HistorialMantencionDesktopLayoutState
    extends State<HistorialMantencionDesktopLayout> {
  late Bus _bus;
        ...
        return _MantencionFormDialog(
          ...
          onGuardar: (Mantencion updatedMantencion) async {
            setState(() {
              ...
              final index = _bus.historialMantenciones
                  .indexWhere((m) => m.id == updatedMantencion.id);
              if (index != -1) {
            ...
            Row(
              ...
              children: [
                Chip(
                  ...
                  backgroundColor:
                      mantencion.completada ? Colors.green : Colors.orange,
                ),
                Text(
...
class __MantencionFormDialogState extends State<_MantencionFormDialog> {
  ...
  void initState() {
    ...
    _descripcionController =
        TextEditingController(text: widget.mantencion?.descripcion ?? '');
    _repuestosController = TextEditingController(
        text: widget.mantencion?.repuestosUsados.join(', ') ?? '');
    _costoController = TextEditingController(
        text: widget.mantencion?.costoTotal?.toString() ?? '');
        ...
        child: Form(
          ...
          child: Column(
            ...
            children: [
              ...
              ListTile(
                title: Text(
                    'Fecha: ${ChileanUtils.formatDate(_fechaSeleccionada)}'),
